...
struct TerminalView<ViewModel: TerminalViewModel>: View {
    ...
    var body: some View {
        ...
        case .ready:
            ZStack {
                VStack(spacing: 0) {
                if viewModel is TerminalController {
                    Divider()
                }
                HStack(spacing: 0) {
                    if let controller = viewModel as? TerminalController {
                        WorkspaceSidebarView(controller: controller, background: ghostty.config.backgroundColor)
                        Divider()
                    }

                    VStack(spacing: 0) {
                        if let controller = viewModel as? TerminalController {
                            TabStripView(controller: controller, background: ghostty.config.backgroundColor)
                            Divider()
                        }
